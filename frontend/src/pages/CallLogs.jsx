...
import { CALL_RESULT_COLORS, CALL_RESULTS, NEXT_ACTIONS, CUSTOMER_STATUSES, formatDate } from '../utils/constants';

export default function CallLogs() {
  const { canManage } = useAuth();
  const qc = useQueryClient();
  const today = new Date().toISOString().split('T')[0];
  const [filters, setFilters] = useState({ call_result: '', sale_id: '', date_from: today, date_to: today, page: 1, limit: 20 });
  const [modal, setModal] = useState(false);
  const [custSearch, setCustSearch] = useState('');
  const [form, setForm] = useState({ customer_id: '', call_date: today, call_time: '', call_result: '', call_content: '', customer_need: '', interest_level: '', next_action: '', follow_up_date: '', status_after_call: '' });
  ...
  const mutCreate = useMutation({
    ...
    onSuccess: () => { toast.success('Đã nhập cuộc gọi'); qc.invalidateQueries(['call-logs']); setModal(false); }
  });
  ...
  return (
    <div className="space-y-4">
      <div className="page-header">
        ...
        <button onClick={() => setModal(true)} className="btn-primary btn-sm">+ Nhập cuộc gọi</button>
      </div>
      ...
      <div className="card">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="flex gap-1 items-center col-span-2 md:col-span-1">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="table-container">
          <table className="table">
            <thead>
              <tr>
                <th>Khách hàng</th><th>Ngày / Giờ</th><th>Kết quả</th>
                <th>Nội dung</th><th>Hành động tiếp</th><th>Follow-up</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="text-center py-10 text-gray-400">Chưa có cuộc gọi nào</td></tr>
              ) : data.data.map(log => (
                <tr key={log.call_id}>
                  <td>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="📞 Nhập kết quả cuộc gọi" size="lg">
        <form onSubmit={(e) => { e.preventDefault(); mutCreate.mutate(form); }} className="space-y-4">
          <div>
            ...
            <input className="input" placeholder="Nhập tên hoặc số điện thoại..." value={custSearch} onChange={e => setCustSearch(e.target.value)} />
